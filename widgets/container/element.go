...
func (e *Element) SetConstraints(c constraints.Constraints) {
	e.Constraints = constraints.Tight(min(e.parent.Width, c.MaxWidth), min(e.parent.Height, c.MaxHeight))
	if e.renderObject != nil {
		e.renderObject.SetConstraints(e.Constraints)
	}
	e.Width, e.Height = min(e.parent.Width, c.MaxWidth), min(e.parent.Height, c.MaxHeight)
}
